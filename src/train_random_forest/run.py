...
def go(args):
    ...
    x_df = pd.read_csv(trainval_local_path)
    y = x_df.pop("price")  # this removes the column "price" from X and puts it into y

    logger.info(f"Minimum price: {y.min()}, Maximum price: {y.max()}")

    x_train, x_val, y_train, y_val = train_test_split(
        x_df, y, test_size=args.val_size, stratify=x_df[args.stratify_by], random_state=args.random_seed
    )
...
def export_model(run, pipe, used_columns, x_val, val_pred, export_artifact):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        mlflow.sklearn.save_model(
            ...
            signature=signature,
            input_example=x_val[used_columns].iloc[:2],
        )
